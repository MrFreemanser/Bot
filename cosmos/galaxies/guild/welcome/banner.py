...
class WelcomeBanner(WelcomeBase):
    ...
    async def welcome_banner_error(self, ctx, error):
        if isinstance(error, exceptions.GuildNotPrime):
            return await ctx.send_line(f"{ctx.emotes.web_emotion.xx}    Get prime to use GIF banners or set static banner.")
    ...
    async def set_welcome_banner(self, ctx, banner_url, channel: typing.Optional[discord.TextChannel] = None, *, text):
        ...
        if banner_size > banner_max_size:
            return await ctx.send_line(f"{ctx.emotes.web_emotion.xx}    Banner should be less than {banner_max_size} MB in size however size of "
    ...
    async def enable_welcome_banner(self, ctx):
        ...
        if ctx.guild_profile.welcome_banner_enabled:
            return await ctx.send_line(f"{ctx.emotes.web_emotion.xx}    Welcome banners are already enabled in {ctx.guild.name}.")
    ...
    async def disable_welcome_banner(self, ctx):
        ...
        if not ctx.guild_profile.welcome_banner_enabled:
            return await ctx.send_line(f"{ctx.emotes.web_emotion.xx}    Welcome banners are already disabled in {ctx.guild.name}.")
